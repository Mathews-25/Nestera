...
fn test_get_top_users_basic() {
    ...
    let top1 = top_users.get(0).unwrap().clone();
    let top2 = top_users.get(1).unwrap().clone();
    let top3 = top_users.get(2).unwrap().clone();
    let top4 = top_users.get(3).unwrap().clone();

    assert_eq!(top1.0, users.get(2).unwrap().clone(), "User 2 should be #1");
    assert_eq!(top1.1, 20_000, "Top user should have 20,000 points");

    assert_eq!(top2.0, users.get(3).unwrap().clone(), "User 3 should be #2");
    assert_eq!(top2.1, 15_000, "Second user should have 15,000 points");

    assert_eq!(top3.0, users.get(0).unwrap().clone(), "User 0 should be #3");
    assert_eq!(top3.1, 10_000, "Third user should have 10,000 points");

    assert_eq!(top4.0, users.get(1).unwrap().clone(), "User 1 should be #4");
...
fn test_get_top_users_limit() {
    ...
    let top1 = top_users.get(0).unwrap().clone();
    assert_eq!(top1.0, users.get(4).unwrap().clone(), "User 4 should be #1");
...
fn test_get_user_rank_not_ranked() {
    ...
    let rank = client.get_user_rank(&users.get(0).unwrap().clone());
...
fn test_get_user_rank_basic() {
    ...
    let rank0 = client.get_user_rank(&users.get(0).unwrap().clone());
    let rank1 = client.get_user_rank(&users.get(1).unwrap().clone());
    let rank2 = client.get_user_rank(&users.get(2).unwrap().clone());
    let rank3 = client.get_user_rank(&users.get(3).unwrap().clone());
...
fn test_get_user_ranking_details() {
    ...
    client.deposit_flexi(&users.get(0).unwrap().clone(), &1000); // 10,000 points
    client.deposit_flexi(&users.get(1).unwrap().clone(), &2000); // 20,000 points
    ...
    let details1 = client.get_user_ranking_details(&users.get(1).unwrap().clone());
...
fn test_ranking_with_ties() {
    ...
    client.deposit_flexi(&users.get(0).unwrap().clone(), &1000); // 10,000 points
    client.deposit_flexi(&users.get(1).unwrap().clone(), &1000); // 10,000 points
    client.deposit_flexi(&users.get(2).unwrap().clone(), &2000); // 20,000 points
...
fn test_ranking_updates_on_new_deposits() {
    ...
    client.deposit_flexi(&users.get(1).unwrap().clone(), &2000); // Total: 25,000 points

    let rank0_after = client.get_user_rank(&users.get(0).unwrap().clone());
    let rank1_after = client.get_user_rank(&users.get(1).unwrap().clone());
...
fn test_ranking_read_only() {
    ...
    client.deposit_flexi(&users.get(0).unwrap().clone(), &1000);
    ...
    let rank1 = client.get_user_rank(&users.get(0).unwrap().clone());
    let rank2 = client.get_user_rank(&users.get(0).unwrap().clone());
